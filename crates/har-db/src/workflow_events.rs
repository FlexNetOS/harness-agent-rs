...
pub fn to_db_date_param(d: &chrono::DateTime<chrono::Utc>) -> String {
    if get_database_type() == DatabaseType::Sqlite {
        ...
        let s = d.to_rfc3339().replace("T", " ");
        // Find the 'Z' or '+' that ends the timezone part and truncate there.
        if let Some(pos) = s.find(|c: char| c == 'Z' || c == '+') {
            s[..pos].to_string()
        } else {
            s
        }
...
impl SqlWorkflowEventStore {
    ...
    pub async fn create_workflow_event(&self, data: &CreateWorkflowEventData) {
        ...
        let step_index_json = data.step_index.map(|v| json!(v)).unwrap_or_else(|| Value::Null);
        let step_name_json = data.step_name.clone().map(|s| json!(s)).unwrap_or_else(|| Value::Null);
        let data_json = data.data.clone().map(|v| json!(v)).unwrap_or_else(|| json!({}));

        self.insert_event(&id, &data.workflow_run_id, event_type_str, step_index_json, step_name_json, data_json).await;
    }
    ...
    pub async fn create_workflow_event_raw(
        ...
        let step_name_json = step_name.clone().map(|s| json!(s)).unwrap_or_else(|| Value::Null);
        let data_json = data.clone().map(|v| json!(v)).unwrap_or_else(|| json!({}));

        self.insert_event(&id, workflow_run_id, event_type, step_index_json, step_name_json, data_json).await;
    }
    ...
    pub async fn list_workflow_events(
        ...
        for row_val in &result.rows {
            if let Some(row_map) = row_val.as_object() {
                ...
                let id = raw.get("id").and_then(Value::as_str).map(String::from).unwrap_or_default();
                let workflow_run_id_val = raw.get("workflow_run_id")
                    .and_then(Value::as_str)
                    .map(String::from)
                    .unwrap_or_default();
                let event_type = raw.get("event_type").and_then(Value::as_str).map(String::from).unwrap_or_default();
                let step_index = raw.get("step_index").and_then(|v| match v {
                    Value::Number(n) => n.as_u64().map(|v| v as u32),
                    Value::Null => None,
                    _ => None,
                });
                let step_name = raw.get("step_name")
                    .and_then(Value::as_str)
                    .map(String::from);
    ...
    pub async fn get_completed_dag_node_outputs(
        ...
        for row_val in &result.rows {
            if let Some(row_map) = row_val.as_object() {
                let step_name = row_map.get("step_name")
                    .and_then(Value::as_str)
                    .map(String::from);
                ...
                if let Some(output) = parsed.get("node_output").and_then(Value::as_str) {
                    ...
                    outputs.insert(step_name, serde_json::to_string(data_val).unwrap_or_default());
                }
            }
        }
...
pub async fn create_workflow_event(
    ...
    store.create_workflow_event_raw(workflow_run_id, event_type, step_index, step_name, data).await;
}
...
pub async fn list_recent_events(
    ...
    let store = SqlWorkflowEventStore::new(get_database().await?);
    store.list_recent_events(workflow_run_id, event_types, limit).await
}
...
mod tests {
    ...
    async fn insert_test_event(
        ...
        let step_index_json = row.step_index.map(|v| json!(v)).unwrap_or_else(|| Value::Null);
        let step_name_json = row.step_name.clone().map(|s| json!(s)).unwrap_or_else(|| Value::Null);
    ...
    async fn create_workflow_event_inserts_correctly() {
        ...
        store.create_workflow_event_raw(
            "run-create-01",
            "step_started",
            Some(1u32),
            Some("build".into()),
    ...
    async fn list_workflow_events_returns_all_for_run() {
        ...
        insert_test_event_parsed(&store, "step_started", &json!({"msg": "start"}).as_object().unwrap().clone(), Some(0), Some("init")).await;
        insert_test_event_parsed(&store, "step_completed", &json!({"output": "ok"}).as_object().unwrap().clone(), Some(1), Some("build")).await;
        insert_test_event_parsed(&store, "node_completed", &json!({"node_output": "final"}).as_object().unwrap().clone(), None, Some("compile")).await;
    ...
    async fn parse_event_row_valid_data_passes_through() {
        ...
        let events = store.list_workflow_events("run-parse-test-2").await.unwrap();
        assert_eq!(events.len(), 1);
    ...
    fn to_db_date_param_sqlite_format() {
        // Force SQLite mode for this test.
    ...
    fn to_db_date_param_postgres_iso_format() {
        // Force Postgres mode (set DATABASE_URL).
    ...
    async fn get_completed_dag_node_outputs_returns_map() {
        ...
        node1.insert("nodeId".into(), json!("node-1"));
        node1.insert("nodeOutput".into(), json!("output-value-1"));

        let mut node2 = Map::new();
        node2.insert("nodeId".into(), json!("node-2"));
        node2.insert("nodeOutput".into(), json!("output-value-2"));
        ...
        let outputs = store.get_completed_dag_node_outputs("run-dag-test").await.unwrap();
        assert_eq!(outputs.len(), 2);
        assert_eq!(outputs.get("node-1-step"), Some(&"output-value-1".to_string()));
        assert_eq!(outputs.get("node-2-step"), Some(&"output-value-2".to_string()));
    }
    ...
    async fn get_completed_dag_node_outputs_uses_node_skipped_prior_success() {
        ...
        skipped.insert("nodeId".into(), json!("skipped-node"));
        skipped.insert("nodeOutput".into(), json!("cached-output"));
        ...
        let outputs = store.get_completed_dag_node_outputs("run-dag-skipped").await.unwrap();
        assert_eq!(outputs.len(), 1);
        assert_eq!(outputs.get("skipped-step"), Some(&"cached-output".to_string()));
    }
    ...
    async fn get_completed_dag_node_outputs_handles_malformed_data() {
        ...
        let outputs = store.get_completed_dag_node_outputs("run-dag-bad-data").await.unwrap();
    ...
    async fn get_completed_dag_node_outputs_empty_for_no_events() {
        let (store, _tmp) = make_test_store();
        let outputs = store.get_completed_dag_node_outputs("run-no-events").await.unwrap();
    ...
    async fn list_recent_events_filters_by_event_types() {
        ...
        insert_test_event_parsed(&store, "node_completed", &json!({"nodeId": "n1"}).as_object().unwrap().clone(), None, Some("n1")).await;
        insert_test_event_parsed(&store, "error", &json!({"msg": "oops"}).as_object().unwrap().clone(), None, Some("err")).await;

        let events = store.list_recent_events(
            "run-test-001",
            Some(vec!["node_completed".into()]),
            10,
        ).await.unwrap();
    ...
    async fn list_recent_events_limit_applies() {
        ...
        let events = store.list_recent_events("run-test-001", None, 3).await.unwrap();
        assert_eq!(events.len(), 3);
    }
    ...
    async fn list_recent_events_no_filter_returns_all() {
        ...
        let events = store.list_recent_events("run-test-001", None, 10).await.unwrap();
        assert_eq!(events.len(), 3);
    }
    ...
    async fn list_events_since_returns_events_after_cursor() {
        ...
        let to_sqlite_date = |dt: &chrono::DateTime<chrono::Utc>| -> String {
            dt.to_rfc3339().replace("T", " ").trim_end_matches('Z').to_string()
        };
        ...
        let events = store.list_events_since("run-since-test", &old_time).await.unwrap();
    ...
    async fn list_events_since_only_future_events() {
        ...
        let events = store.list_events_since("run-since-future", &now).await.unwrap();
