...
impl InMemStore {
    fn new(fx: Arc<Mutex<Fx>>) -> Arc<Self> {
        Arc::new(Self { fx, ..Default::default() })
    }
}
...
fn mk(
    ...
    IsolationResolver::new(IsolationResolverDeps {
        store,
        provider: Arc::new(MockProvider { fx, create_branch: branch.to_string() }),
        cleanup: None,
...
async fn seed(store: &Arc<dyn IsolationStore>, wt: IsolationWorkflowType, wid: &str, path: &str, branch: &str) -> String {
    store
...
async fn fail1_stale_calls_store_update_status_not_provider_destroy() {
    ...
    let id = seed(&store, IsolationWorkflowType::Thread, "w1", "/totally/missing/wt", "archon/thread-w1").await;
    ...
    assert!(matches!(res, IsolationResolution::StaleCleaned { ref previous_env_id } if *previous_env_id == id));
    let g = fx.lock().unwrap();
    // TS oracle: updateStatusCalls = [{id, "destroyed"}], destroyCalls = []
    assert_eq!(g.update_status_calls, vec![(id.clone(), "destroyed".to_string())], "must mark DB row destroyed");
    assert!(g.destroy_calls.is_empty(), "must NOT physically destroy worktree, got {:?}", g.destroy_calls);
}
...
async fn fail5_cross_clone_reuse_propagates_no_create() {
    ...
    for repo in [&repo1, &repo2] {
        let out = Command::new("git").args(["init", "-q", "-b", "main", repo]).output().unwrap();
    ...
    git(&repo2, &["worktree", "add", "-q", "-b", "cross-branch", &wt2]);

    let fx = Arc::new(Mutex::new(Fx::default()));
    let store: Arc<dyn IsolationStore> = InMemStore::new(fx.clone());
    // active workflow row for cdb-001 (repo1) points at repo2's worktree
    seed(&store, IsolationWorkflowType::Thread, "wX", &wt2, "cross-branch").await;
    fx.lock().unwrap().create_calls = 0;
    ...
    assert!(res.is_err(), "cross-clone reuse must propagate Err, got {res:?}");
    let g = fx.lock().unwrap();
    assert_eq!(g.create_calls, 0, "must NOT fall through to create on cross-clone conflict");
}
...
async fn fail3_no_basebranch_hint_emits_no_warning() {
    ...
    let fx = Arc::new(Mutex::new(Fx::default()));
    let store: Arc<dyn IsolationStore> = InMemStore::new(fx.clone());
    seed(&store, IsolationWorkflowType::Thread, "w1", &wt, "feature/x").await;
    ...
    match res {
        IsolationResolution::Resolved(p) => {
            assert!(p.warnings.is_none(), "no-hint must emit no warnings, got {:?}", p.warnings);
        }
...
async fn fail4_suggested_branch_only_does_not_adopt() {
    ...
    match res {
        IsolationResolution::Resolved(p) => {
            assert!(
                matches!(p.method, har_isolation::types::ResolutionMethod::Created { .. }),
                "suggestedBranch-only must fall to create, got {:?}", p.method
            );
        }
...
impl IsolationStore for FailingCreateStore {
    ...
    async fn find_active_by_workflow(&self, _c: &str, _w: IsolationWorkflowType, _i: &str)
        -> har_isolation::Result<Option<IsolationEnvironmentRow>> { Ok(None) }
    async fn create(&self, _e: CreateEnvironmentParams) -> har_isolation::Result<IsolationEnvironmentRow> {
        Err(har_isolation::IsolationError::Other("DB write failed (simulated)".into()))
    }
    async fn update_status(&self, id: &str, status: &str) -> har_isolation::Result<()> {
        self.fx.lock().unwrap().update_status_calls.push((id.to_string(), status.to_string()));
        Ok(())
    }
    async fn count_active_by_codebase(&self, _c: &str) -> har_isolation::Result<u32> { Ok(0) }
}
...
async fn stage6_orphan_cleanup_uses_provider_destroy_not_store_update() {
    ...
    assert!(res.is_err(), "store-create failure must propagate, got {res:?}");
    let g = fx.lock().unwrap();
    ...
    assert!(g.update_status_calls.is_empty(),
        "orphan cleanup must NOT call store.update_status; got {:?}", g.update_status_calls);
}
