...
use har_isolation::types::{
    ...
    ResolutionMethod, ResolveRequest, WorktreeEnvironment, WorktreeMetadata,
};
...
fn method_str(r: &IsolationResolution) -> String {
    match r {
        IsolationResolution::Resolved(p) => match &p.method {
            ...
            ResolutionMethod::LinkedIssueReuse { issue_number } => {
                format!("resolved/linked_issue_reuse:{issue_number}")
            }
            ResolutionMethod::BranchAdoption { branch } => {
                format!("resolved/branch_adoption:{branch}")
            }
...
async fn stage1_existing_resolved() {
    ...
    let id = {
        let row = store
            .find_active_by_workflow("cdb-001", IsolationWorkflowType::Thread, "w1")
            .await
            .unwrap()
            .unwrap();
        row.id
    };
    let res = resolver(store, &r.branch)
        .resolve(ResolveRequest {
            existing_env_id: Some(id),
            codebase: Some(cb(&r.repo)),
            hints: None,
...
async fn stage1b_missing_worktree_is_stale_cleaned() {
    ...
    let id = store
        ...
        .unwrap()
        .id;
    let res = resolver(store, "archon/thread-w1")
        .resolve(ResolveRequest {
            existing_env_id: Some(id),
            codebase: Some(cb(&r.repo)),
            hints: None,
...
async fn stage3_basebranch_mismatch_warning_text() {
    ...
    let res = resolver(store, &r.branch)
        .resolve(ResolveRequest {
            ...
            hints: Some(IsolationHints {
                workflow_type: Some(IsolationWorkflowType::Thread),
                workflow_id: Some("w1".into()),
                base_branch: Some("nonexistent-base".into()),
                ..Default::default()
            }),
            platform_type: "slack".into(),
            user_id: None,
    ...
    if let IsolationResolution::Resolved(p) = &res {
        ...
        panic!("expected resolved, got {res:?}");
    }
}
...
async fn stage4_linked_issue_reuse() {
    ...
    assert_eq!(
        method_str(&res),
        "resolved/linked_issue_reuse:99",
        "stage4 linked"
    );
}
...
async fn stage5_branch_adoption() {
    let r = setup_repo();
    let store: Arc<dyn IsolationStore> = InMemStore::new();
    let res = resolver(Arc::clone(&store), &r.branch)
        .resolve(ResolveRequest {
            existing_env_id: None,
            codebase: Some(cb(&r.repo)),
            hints: Some(IsolationHints {
                workflow_type: Some(IsolationWorkflowType::Pr),
                workflow_id: Some("pr1".into()),
                pr_branch: Some(r.branch.clone()),
                ..Default::default()
            }),
            platform_type: "slack".into(),
            user_id: None,
    ...
    assert_eq!(
        method_str(&res),
        format!("resolved/branch_adoption:{}", r.branch),
        "stage5 adopt"
    );
    // TS golden: store.create called with metadata {adopted:true, adopted_from:'skill'}
    let row = store
        .find_active_by_workflow("cdb-001", IsolationWorkflowType::Pr, "pr1")
        .await
        .unwrap()
        .expect("adoption should persist a row");
    let meta = serde_json::to_value(&row.metadata).unwrap();
    assert_eq!(
        meta.get("adopted").and_then(|v| v.as_bool()),
        Some(true),
        "adopted flag"
    );
    assert_eq!(
        meta.get("adopted_from").and_then(|v| v.as_str()),
        Some("skill"),
        "adopted_from"
    );
}
...
async fn stage6_create_new() {
    let r = setup_repo();
    let store: Arc<dyn IsolationStore> = InMemStore::new();
    let res = resolver(store, "archon/thread-fresh")
        .resolve(ResolveRequest {
            existing_env_id: None,
            codebase: Some(cb(&r.repo)),
            hints: Some(IsolationHints {
                workflow_type: Some(IsolationWorkflowType::Thread),
                workflow_id: Some("fresh".into()),
                ..Default::default()
            }),
            platform_type: "slack".into(),
            user_id: None,
