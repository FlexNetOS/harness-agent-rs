...
async fn drive(provider: &ClaudeProvider, cancel: Arc<dyn CancelToken>) -> Vec<MessageChunk> {
    provider
        .send_query("p".to_owned(), "/tmp".to_owned(), None, None, cancel)
        .collect()
        .await
}
...
async fn happy_path_single_attempt() {
    ...
    assert!(chunks
        .iter()
        .any(|c| matches!(c, MessageChunk::Assistant { content, .. } if content == "hi")));
    assert!(chunks
        .iter()
        .any(|c| matches!(c, MessageChunk::Result { session_id: Some(s), .. } if s == "s1")));
}
...
async fn crash_then_success_retries_once() {
    ...
    assert!(chunks
        .iter()
        .any(|c| matches!(c, MessageChunk::Assistant { content, .. } if content == "recovered")));
}
...
async fn crash_exhausts_retries_four_attempts() {
    ...
    assert_eq!(
        sp.count(),
        4,
        "crash should retry to MAX (4 total attempts: 0,1,2,3)"
    );
    assert!(!chunks
        .iter()
...
async fn backoff_delay_applied_before_retry() {
    ...
    assert!(
        ...
        "backoff delay (40ms) must be applied, elapsed={:?}",
        elapsed
    );
...
async fn cancel_before_attempt_zero_spawns() {
    let sp = CountingSpawner::new(vec![Script::Success(vec![
        assistant("never"),
        result("s4"),
    ])]);
    let provider = ClaudeProvider::new_for_test(sp.clone());
...
async fn first_event_timeout_terminates_without_retry() {
    ...
    assert_eq!(
        sp.count.load(Ordering::SeqCst),
        1,
        "timeout must NOT retry (1 spawn)"
    );
    assert!(
        elapsed.as_millis() >= 45,
        "timeout (~50ms) should gate the wait, elapsed={:?}",
        elapsed
    );
