...
- Sub-cycle 2: `executeDagWorkflow` orchestrator (~960 ln) — [x] `- [x]` cycle 33 (see below). **WHOLE-FUNCTION ROLLUP CONFIRMED cycle 42 (parity-verifier):** after 4f the dispatch `match &node_owned` is exhaustive over all 7 DagNode variants with no Skipped `_` placeholder; every arm (Bash/Cancel 4a, Script 4b, Command/Prompt 4d, Loop 4e, Approval 4f) + the orchestrator layer logic is parity-verified → `executeDagWorkflow` whole-function symbol flipped `- [x]` in symbol-map.
...
- Sub-cycle 4: node-type execution dispatch — DECOMPOSED by architect (cycle 36) into 4a–4f (findings/WF-09-s4-architecture.md):
  ...
  - 4c AI-node live streaming (executeNodeInternal body + with_idle_timeout + validate_structured_output) — **[x] cycle 39, parity-verified vs live bun + live node** (findings/parity-WF-09-s4c.md; gate FAILed first pass on D1 idle-minute integer-division rendering [`90000ms → "1 min"` vs TS `"1.5 min"`] → porter ported a faithful ECMA-262 `format_js_number(f64)` + `idle_timeout_minutes(Duration)`, fixed all 3 sites [rs:5448/5473/5594], cross-checked byte-identical vs live `node -e 'String(x)'` across whole-minute/fractional/tiny/exponential regimes; re-verify 11/11 differential probes PASS, clippy --all-targets clean). Complete port: idle-timeout watchdog (`tokio::time::timeout` re-arm per chunk = `withIdleTimeout`), `'reask: loop` with per-pass accumulator reset, all per-chunk dispatch branches (Assistant/Tool/ToolResult/Result/System/catchall), tool_started/tool_completed pairing via `LastToolStart`, cancel-check (10s) + heartbeat (60s) via `OnceLock<Mutex<HashMap>>` throttle maps, `DagNodeCancelToken` orphan-rule bridge, `validate_structured_output` (real call, fail-safe on uncompilable schema), `build_reask_prompt`/`emit_reask`, batch flush via `safe_send_message`, idle-timeout notice, cancelled→Failed path with `cleanup_throttle_maps`, credit-exhaustion detection, empty-output detection, success path with `declared_fields_from_schema`. 13 helper/constant tests pinned in `sub_cycle4c_tests`. Full differential streaming test requires live `AgentProvider` — deferred to parity-verifier scope. Build: 0 errors, clippy --all-targets clean.
  - 4d AI-node dispatch wiring + retry wrapper + session persist — **[x] cycle 40, parity-verified vs live bun** (findings/parity-WF-09-s4d.md). Command/Prompt arm wired to execute_node_internal with resolve_node_provider_and_model, retry loop (exponential backoff, fatal/transient classification incl. FATAL-never-retries-under-`all`), persist_session upsert/delete; Loop/Approval stay honest-Skipped (4e-4f). Gate FAILed first pass on 3 divergences (D-1 delete-session error branch swallowed via `let _=`; D-2 AI-node cost dropped at NodeOutput mapping → completion `total_cost_usd` never emitted; D-3 pre-exec error event omitted `nodeName`); porter fixed all 3 (D-1 delete error now warns byte-identical to TS:3380; D-2 spawned-task return widened to `(String,NodeOutput,Option<f64>)` + `total_cost_usd` made `mut` and accumulated per TS:3427/:3651; D-3 emits `command ?? id` per TS:3404), re-verified independently: 8/8 probes PASS, durable gate `cycle9_4d_parity_gate.rs` (4 tests: D-1/D-2/D-2b-multi-node/D-3, observable) + 6 tests in cycle9_4d_ai_dispatch.rs all pass; crate suite 425 passed; clippy --all-targets clean.
  - 4e loop node — **[x] cycle 41 — PARITY-VERIFIED vs live bun 1.3.14 (findings/parity-WF-09-s4e.md)**. Gate PASS first pass (no divergence): all 11 contract branches + every pure-string oracle (gate-message ⏸/backticks, completion plural, SDK double-wrap, empty/max/stop strings, `startIteration=(iter??0)+1`, `cleanOutput||fullOutput`, 500-UTF-16 truncation) re-derived from running TS and matched byte-for-byte; `run_subprocess` env inheritance (`...process.env`+8 loop keys+`config.envVars` LAST) confirmed faithful via `.env_clear().envs(std::env::vars()).envs(overlay)`; 4 porter-proposed `[≈]` all adjudicated faithful (no `[≠]`, no disguised feature-skip). Added 3 durable verifier differential tests (`tests/cycle9_4e_loop_verify.rs`: tool_input 500-UTF-16 truncation byte-exact, until_bash `config.envVars` overlay-wins-LAST + 8-key plumbing, exit-nonzero-continues + LOOP_PREV_OUTPUT prev-cleaned-output threading) — all green; clippy --tests clean. `execute_loop_node` (dag_executor.rs) + the `DagNode::Loop` dispatch arm (resolve_node_provider_and_model → execute_loop_node → NodeOutput map + cost capture). Full port of TS:1955-2558: provider fail-fast (via capability registry; Rust `get_agent_provider` seam is infallible), interactive-resume detection from `metadata.approval`/`loop_user_input` (is_approval_context), `for i in start..=max_iterations` with between-iteration status check (`should_continue_streaming_for_status`; paused tolerated H4), loop_iteration_started event, session threading (`fresh_context || i==1`), per-iteration stream reusing 4c's `with_idle_timeout` re-arm + tool-event pairing + result capture (session/cost+/stop/turns+/structured) + SDK-error throw (`subtype!='success'`) via labeled `'body` block, per-iteration catch→Failed (double-wrapped error string faithful to TS:2270), idle-timeout notice (reuses `idle_timeout_minutes`/D1 fix), empty-output guard, batch send, `detect_completion_signal`, `until_bash` real subprocess via `run_subprocess` (D3) with 8-key env + `env_vars` LAST, loop_iteration_completed event + log_node_complete, completion exit (interactive-first-run gating ts:2439 → node_completed event + Completed), interactive gate (approval_requested + `pause_workflow_run` InteractiveLoop ApprovalContext + approval_pending + Completed; gate-delivery-fail→Failed), max-iterations→Failed. Loop tool_input 500-UTF-16-unit truncation (TS:2221-2228, distinct from executeNodeInternal's raw input). 11 Fake-seam self-tests (`tests/cycle9_4e_loop.rs`) drive the REAL `execute_loop_node` through scripted-provider per-iteration scripts: completion-via-signal, completion-via-until_bash-exit0 (real bash), max-iterations, empty-output, SDK-error, between-iter-stop, paused-tolerated (H4), interactive-gate-first-run, interactive-resume, fresh_context-no-resume, batch-send. Build: 0 errors; clippy --workspace --all-targets -D warnings clean; crate suite 366 lib + all integration green (no regression). **Awaiting differential parity-verifier gate.** Findings: findings/WF-09-s4e-port.md. Approval still honest-Skipped (4f).
  - 4f approval node (+ on_reject reuse) — **[x] cycle 42 — PARITY-VERIFIED vs live bun 1.3.14 (findings/parity-WF-09-s4f.md; PASS first pass, no divergence)**. All 7 probe-battery branches differentially confirmed via the porter's 5 Fake-seam tests (driving the REAL `execute_dag_workflow`) + a new verifier durable test for the untested pre-exec-failure path (`tests/cycle9_4f_approval_verify.rs`: on_reject `$BASE_BRANCH`-empty substitute throw → `approval_pre_exec_failure` → Failed + node_failed event + "failed before execution" msg + no pause/no cancel/no AI run). All 4 pure-string oracles re-derived from running TS under bun and matched byte-for-byte (⏸ U+23F8 gate message + Run-ID/approve/reject template; ❌ U+274C cancel message; `max_attempts (N) exhausted` reason for N∈{3,5,10}; synthetic `${id}:on_reject` id; `?? 3`/`?? 0` defaults; float `2.5>=3` compare). 2 `[≈]` adjudicated faithful (no `[≠]`, no disguised feature-skip): (a) `approval_pending` emitter message-slot is the shared WF-15 emitter gap — the message is fully observable via the `approval_requested` store event + the gate message, durable state is full-fidelity; (b) `onRejectMaxAttempts` f64-vs-int serialization is the inherited WF-06 `ApprovalContext` struct decision (TS `number`→`f64`, identical to the already-verified `iteration` field), never parsed back as an int by any consumer, faithful pass-through via `.map(f64::from)` — not 4f-introduced. crate suite 445 passed (+1 verifier test); clippy --workspace --all-targets clean. `execute_approval_node` (dag_executor.rs) + the `DagNode::Approval` dispatch arm + a new `approval_pre_exec_failure` helper that replicates the `executeDagWorkflow` dispatch catch (TS:3387-3416) for the throw-equivalent ops (substitute / resolve / pause). Full port of TS:2565-2747: rejection-resume detection from `metadata.approval`+`rejection_reason` gated on `is_approval_context` + `type==='approval'` + `nodeId===node.id` (TS:2588-2598); on_reject path (TS:2601-2702) — `max_attempts ?? 3` exhaustion → `cancel_workflow_run` + workflow_cancelled store event + `{type,runId,nodeId,reason}` emitter + cancel message + return Completed (TS:2606-2630); else `substitute_workflow_variables(rejectionReason, shellSafe=false)` + synthetic `${id}:on_reject` PromptNode (copies depends_on+idle_timeout only) + `resolve_node_provider_and_model` + `execute_node_internal` (fresh session) + Failed-passthrough + fall-through (TS:2632-2701); standard gate (TS:2704-2746) — `substitute_node_output_refs(message)`, ⏸ approval-gate message, `approval_requested` typed store event, `pause_workflow_run({type:'approval',captureResponse,onRejectPrompt,onRejectMaxAttempts})`, `approval_pending` emitter, return Completed{empty}. **After 4f the dispatch placeholder is FULLY removed** — the `match &node_owned` is exhaustive over all 7 DagNode variants (Bash, Cancel, Script, Command, Prompt, Loop, Approval); the honest-Skipped `_ =>` arm is deleted (no variant remains on a placeholder). 5 Fake-seam self-tests (`tests/cycle9_4f_approval.rs`) drive the REAL `execute_dag_workflow` through the approval arm via scripted-provider + recording store: standard-gate pause shape, capture_response+on_reject context threading, on_reject AI re-run + re-pause + synthetic-id non-collision (asserts `node_completed` fires for `gate:on_reject` and NEVER for `gate`), max_attempts exhaustion → cancel (no pause, no AI), mismatched-metadata falls through to standard gate. Build: 0 errors; clippy --workspace --all-targets -D warnings clean; crate suite 444 passed (no regression). Feeds sub-cycle 5 = whole-DAG differential harness + pre-DONE WF-09 left-behind sweep.
- Sub-cycle 5: whole-DAG differential harness + ≠2 web send_structured_event confirmation + pre-DONE WF-09 left-behind sweep:
  - **5A** whole-DAG END-TO-END differential harness — **[x] cycle 43 — PARITY-VERIFIED vs live bun 1.3.14 (findings/parity-WF-09-s5-wholedag.md)**. FIRST integration-grade proof of the whole executor: 3 composed multi-node workflows (multi-layer topological ordering, node-output refs threaded BETWEEN nodes, trigger/`when` gating across layers, bash→AI→loop→approval→cancel mix) driven through BOTH live bun `executeDagWorkflow` (oracle, `meta-yard/Archon/packages/workflows/src/wholedag-oracle.test.ts` → golden `tests/fixtures/cycle9_5_wholedag_oracle.json`) AND the Rust `execute_dag_workflow`, diffed field-by-field on final node-output map + full ordered event stream + store mutations + platform sends. Differential started **RED (19→16 divergences)** — exactly the COMPOSITION bugs single-node probes (4a–4f) structurally cannot see — and verifier fixed **7 real divergence classes** in dag_executor.rs: (1) **CRITICAL** trigger-rule + `when:` evaluated against a minimal map (node's own prior only) vs the prior-LAYER snapshot → every downstream node wrongly `skipped(trigger_rule)`, whole multi-layer DAG collapsed, AI nodes never ran → build shared `all_outputs` view before gating; (2) removed invented `workflow_started` event (TS emits none); (3) `workflow_completed` store event bogus `step_name` → workflow-level; (4) workflow-level messages mis-routed through store `workflow_artifact` carrier instead of `platform.send_message` (D1 mis-routing) + status casing `Cancelled`→`cancelled` → routed to platform + lowercased; (5) AI `node_started` `command` `"<inline>"` vs TS `null`; (6) AI `node_completed` dropped `stop_reason`/`num_turns`/`model_usage` (Result chunk bound to `_`) → captured + conditionally included; (7) `cost_usd` emitted `0` when absent vs TS omit (Option accumulator) + `format_tool_call` returned `None` vs TS JSON-default fall-through (+ `mcp__` split fix). **≠2 CONFIRMED:** `send_structured_event` default no-op is faithful (no web adapter ported); recording platform overrode it and captured the `tool`+`tool_result` chunks fired at AI-node call sites byte-for-byte vs bun — seam wired correctly; real web/SSE override stays **owed by WF-32 (deps.ts) / SV-03 (web.ts)** [both `- [ ]`]. Durable: `tests/cycle9_5_wholedag.rs`. Gate: crate 446/446 + workspace 2182 passed/0 failed; clippy --workspace --all-targets clean.
  - **5B** pre-DONE WF-09 full re-harvest / left-behind sweep — **RAN cycle 44 (cartographer, opus); VERDICT NOT-READY (findings/WF-09-s5b-sweep.md)** — the completeness gate caught GENUINE left-behind behavior no sub-cycle gate (4d/4e/4f/5A) targeted. Re-harvest: git-kb index was STALE (2026-06-05, capped at ln 3495 vs 3710) → re-indexed → full WF-09 source surface = 28 fn/iface/type + 8 const + 3 module-cache. **14 source symbols had NO symbol-map row** (silent left-behind class) — all 14 added; most faithfully ported + covered by a verified parent gate (`shell_quote`/retry helpers/`run_stream_pass`/`skip_if_status_changed`/throttle-maps → `[x]`) or pure idiom (`getLog`/`cachedLog`/`logEventStoreError` → `[≈]`). But 2 of the missing — **`resolveNodeProviderAndModel` (rs:567, live path :3851 AI + :4129 loop) + `applyPresetOptions` (rs:872, dead code)** — hid **7 real dropped behaviors (G1–G7)**: G1 capability checks ignore actual provider caps (`_caps` unused rs:650; warns on EVERY set field rs:696 vs TS `isSet && !caps[cap]`) → FALSE warnings + effort/thinking use wrong stand-in; G2/G3 capability-warning USER delivery via `safe_send_message` + delivery-fail log DROPPED (log-only); G4 model/provider-conflict USER delivery DROPPED (rs:610 "skip in utility-only scope"); G5 agents+skills-collision USER message DROPPED (rs:712); G6 preset thinking/effort cascade NEVER applied (`apply_preset_options` `#[allow(dead_code)]`, inline `let _ = thinking` rs:766); G7 node `hooks` serialization into node_config DEFERRED (rs:754) → silently dropped. Deps (`safe_send_message` WF-11 `[x]` + WorkflowPlatform seam built 4a) ALREADY available → WF-09's OWN port-incompleteness, NOT a sibling deferral. Also: 1 phantom target (`scheduleReask`→`schedule_reask()` doesn't exist, behavior faithfully INLINED rs:5891/5930, 4c-covered → flip `[≈]`), 1 wrong citation (`NodeState` 0× in dag-executor.ts = WF-06 enum mirrored rs:4748 → flip `[x]`), `NodeExecutionResult`→real struct rs:4729 → flip `[x]`. **Rollup violation:** `executeDagWorkflow` `[x]` calls now-`[!]` resolve helper. Sibling deferrals cross-ref'd (web `send_structured_event`→WF-32/SV-03; WF-15 emitter slot). **→ cycle 45 fixes G1–G7 + flips the 3 `[~]` rows, then re-run 5B.**
  - **5B-resolve** G1–G7 + #2 + #5 differential parity gate — **[x] cycle 45 — PASS, PARITY-VERIFIED vs live bun 1.3.14 (findings/parity-WF-09-s5b-resolve.md)**. The verifier built its OWN bun oracle (not the porter's reported values): warning strings byte-checked against the TS templates (TS:434/505/525); the claude+codex `ProviderCapabilities` confirmed byte-equal to the REAL `packages/providers/src/{claude,codex}/capabilities.ts` (not the test mocks); `CLAUDE_EFFORTS`/`CODEX_REASONING_EFFORTS` + `routePresetEffort` byte-equal; the `{...baseOptions, nodeConfig, assistantConfig}` embedding confirmed consumed at BOTH dispatch sites (`resolved.base_options.clone()`→provider; top-level `node_config`/`assistant_config` are `None` and unread → no double-wrap); agents copied field-by-field (no serde round-trip dropping disallowedTools/maxTurns). G1–G7 all confirmed. **#2** status-helper reuse confirmed. **Verifier-FIXED nuance 2 (real divergence):** a FATAL warning-delivery error was swallowed by `.unwrap_or(false)`; now propagates as a resolve `Err("Platform authentication/permission error: …")` at all 3 delivery sites (G2/G4/G5), matching TS `safeSendMessage` rethrow (executor-shared.ts:632-634) → dispatch reproduces TS's "failed before execution" node-failure. **Nuance 1 adjudicated faithful `[≈]`** (WLO population deferred to `execute_dag_workflow` signature / SV-01; precedence logic complete). **Code-review #5 coverage ADDED:** a 4th "costomit" whole-DAG workflow (completing AI node, cost OMITTED) on BOTH the bun oracle + the Rust differential (golden regenerated from live bun) — a mutation (`cost_usd: 0.0` instead of omit) was proven to FAIL the gate (discriminating). Tests: `tests/cycle9_5b_resolve_gaps.rs` (10 probes), `tests/cycle9_5_wholedag.rs` (+costomit). Gate: `cargo test -p har-dag-executor` 457 passed / 0 failed; clippy --all-targets clean. **WF-09 is DONE-READY** — all WF-09 symbols `[x]`/`[≈]`, zero `[ ]`/`[~]`/`[!]`, rollup restored (`executeDagWorkflow` now calls a verified `[x]` resolve helper).
...
- [x] `executeNodeInternal(...)` — AI (command/prompt) node: idle-timeout abort controller, session fork on resume, validate-and-reask loop (up to `STRUCTURED_OUTPUT_MAX_REASKS=3` for best-effort providers), streaming/batch mode, cancel-during-streaming detection every 10s (`CANCEL_CHECK_INTERVAL_MS`), activity heartbeat every 60s, credit-exhaustion detection, empty-output detection, MCP failure filtering, structured-output override, tool event emission (dag-executor.ts:672-1490). **[x] cycle 39 — 4c live-streaming body parity-verified vs live bun** (findings/parity-WF-09-s4c.md); every branch from TS source lines 672-1490 ported and driven through 11 scripted-fake-provider differential probes (assistant/batch/tool-pairing/result/cost-cap/SDK-error/MCP-filter/structured-reask/idle-H1/cancel-H2/paused-H4/credit/empty/success). D1 idle-minute float-rendering divergence caught + fixed (`format_js_number`). The AI *dispatch arm* in execute_dag_workflow remains honest-Skipped → sub-cycle 4d.
...
- [x] `executeLoopNode(...)` — iterative AI loop: max_iterations guard, fresh vs shared context per iteration, completion signal detection via `detectCompletionSignal`, bash condition `until_bash` exit code check, interactive-loop gate (pause + await user input via `/workflow approve`), loop resume from `metadata.approval`, empty-output failure per iteration, cost accumulation across iterations (dag-executor.ts:1955-2558) — **PARITY-VERIFIED cycle 41 (4e) vs live bun, findings/parity-WF-09-s4e.md**
- [~] `executeApprovalNode(...)` — human-in-loop gate: `on_reject` cycle (re-run AI with rejection reason; `max_attempts` check → cancel after exhaustion), `capture_response` flag, `pauseWorkflowRun` call, re-pause loop (dag-executor.ts:2565-2747) — **cycle 42 PORTED as `execute_approval_node` + `DagNode::Approval` dispatch arm (parity unproven; awaiting verifier gate)**. See sub-cycle-4f note above. After this, the dispatch placeholder is fully removed (match exhaustive over 7 variants).
...
- [x] `when:` condition evaluation via `evaluateCondition` before trigger-rule check — verified 5A (the prior-LAYER snapshot fix; whole-DAG differential)
- [≈] Workflow-level options (`effort`, `thinking`, `fallbackModel`, `betas`, `sandbox`) cascade to node unless node overrides — verified 5B: `node.x ?? wlo.x` precedence faithful + differentially exercised; `fallbackModel` populated, effort/thinking/betas/sandbox population deferred to the `execute_dag_workflow` workflow-options signature (SV-01), tracked

**Error paths:**
- [x] Unknown provider at node level → descriptive error listing all registered providers (dag-executor.ts:450-457) — verified 5B (resolve registered-provider check; byte-matched message format)
- [x] Model/provider conflict → warning message + proceed with resolved provider (dag-executor.ts:421-436) — verified 5B (G4; `safe_send_message` delivery byte-matched + delivery-fail log + fatal-rethrow)
- [x] agents+skills ID collision warning (dag-executor.ts:516-528) — verified 5B (G5; `safe_send_message` delivery byte-matched)
- [x] Capability warnings for each unsupported field per provider (dag-executor.ts:471-510) — verified 5B (G1/G2/G3; real cap registry + byte-matched single/plural + delivery-fail log + fatal-rethrow)
- [x] MCP failure prefix filtering: workflow-configured servers surface to user; user-plugin servers suppressed to debug log (dag-executor.ts:1063-1115) — verified 4c
- [x] `error_max_budget_usd` errorSubtype → throw cost-cap error (dag-executor.ts:1021-1029) — verified 4c
- [x] `isError && errorSubtype !== 'success'` → throw SDK error (dag-executor.ts:1039-1054) — verified 4c
- [x] Node idle-timeout with non-empty output → warn and continue; with empty output → fail (dag-executor.ts:1258-1387) — verified 4c (D1 float-render fix)
- [x] Credit exhaustion detected in assistant text → fail node (dag-executor.ts:1317-1349) — verified 4c
- [x] Loop gate-message delivery failure → fail node rather than orphan paused run (dag-executor.ts:2500-2513) — verified 4e
