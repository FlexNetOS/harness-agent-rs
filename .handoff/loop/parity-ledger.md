...
- Sub-cycle 5: whole-DAG differential harness + ≠2 web send_structured_event confirmation + pre-DONE WF-09 left-behind sweep:
  ...
  - **5B** pre-DONE WF-09 full re-harvest / left-behind sweep (`git kb code symbols` over dag-executor.ts vs symbol-map; zero missing / zero `- [ ]`/`- [~]`/`- [!]` / zero rollup violations) — [ ] pending (next cycle; the completeness gate that lets WF-09 → DONE).
