...
- [x] unit:WF-09 `dag-executor.ts::executeDagWorkflow` → `workflows::dag_executor::execute_dag_workflow()` — **ROLLED UP `- [x]` cycle 42** (all dispatch arms + orchestrator now parity-verified). Orchestrator structure parity-verified sub-cycle 2 (cycle 33); dispatch arms: Bash+Cancel verified sub-cycle 4a (cycle 36), Script verified sub-cycle 4b (cycle 37), AI(Command/Prompt) **parity-verified sub-cycle 4d (cycle 40, vs live bun — findings/parity-WF-09-s4d.md; gate FAILed first on D-1 delete-error swallow + D-2 dropped cost + D-3 missing nodeName → all 3 porter-fixed + independently re-verified, 8/8 probes + 4-test durable gate)**; Loop **parity-verified sub-cycle 4e (cycle 41)**; Approval **parity-verified sub-cycle 4f (cycle 42, vs live bun 1.3.14 — findings/parity-WF-09-s4f.md; PASS first pass, no divergence)**. **ROLLUP CONFIRMED:** after 4f the dispatch placeholder is FULLY removed — the `match &node_owned` (dag_executor.rs:3715-4234) is exhaustive over all 7 DagNode variants (Bash, Cancel, Script, Command, Prompt, Loop, Approval); the Skipped `_` arm is gone (verifier read the closing brace at 4234 + GREEN build proves exhaustiveness). Every arm + the orchestrator layer logic verified → whole-function symbol flipped `- [x]` by the parity-verifier.
...
- [x] unit:WF-09 `<D1 idiom: JS String(number) for idle-minute rendering>` → `workflows::dag_executor::format_js_number()` + `idle_timeout_minutes()` — parity-verified sub-cycle 4c (cycle 39; faithful ECMA-262 §6.1.6.1.20 `Number::toString` — shortest digits, fixed/exponential re-layout, four range branches, `-0`/NaN/±Infinity; cross-checked byte-identical vs live `node -e 'String(x)'` incl. exp regime `8.333…e-7`; replaces integer `as_millis()/60_000` at the 3 idle sites that diverged on non-whole-minute timeouts)
- [x] unit:WF-09 `dag-executor.ts::executeNodeInternal` → `workflows::dag_executor::execute_node_internal()` — live-streaming body parity-verified sub-cycle 4c (cycle 39, findings/parity-WF-09-s4c.md; 11 scripted-fake-provider differential probes incl. H1 idle re-arm, H2 cancel, H4 paused, reask, credit-exhaustion; D1 idle-minute float-rendering fixed via `format_js_number` ECMA-262 port, cross-checked vs live node)
- [x] unit:WF-09 `dag-executor.ts::buildReaskPrompt` → `workflows::dag_executor::build_reask_prompt()` — parity-verified sub-cycle 4c (cycle 39; probe 8 reask path)
- [x] unit:WF-09 `dag-executor.ts::emitReask` → `workflows::dag_executor::emit_reask()` — parity-verified sub-cycle 4c (cycle 39; probe 8 reask path)
- [≈] unit:WF-09 `dag-executor.ts::scheduleReask` → behavior INLINED (no `schedule_reask()` symbol) at `dag_executor.rs:5891-5904` (TS:1227) + `:5930-5943` (TS:1241) — **verifier-flipped cycle 45 (parity-WF-09-s5b-resolve.md)**: the phantom-target row corrected per 5B sweep; the reask-scheduling closure is inlined into the `'reask` loop and was parity-verified under sub-cycle 4c (probe 8). `[≈]` = idiom (nested closure inlined), not a gap.
- [x] unit:WF-09 `dag-executor.ts::NodeExecutionResult` → `workflows::dag_executor::NodeExecutionResult` — **verifier-flipped cycle 45 (parity-WF-09-s5b-resolve.md)**: real target struct at `dag_executor.rs:4729`; covered + parity-exercised under 4c/4d (D-2 cost accumulator, state/output/session/error fields).
- [x] unit:WF-06 `schemas/workflow-run.ts::NodeState` (re-cited; 0× in dag-executor.ts) → `har_workflow_schema` enum mirrored as `dag_executor.rs:4748` Rust enum — **verifier-flipped cycle 45 (parity-WF-09-s5b-resolve.md)**: the WF-09 citation was wrong (NodeState is WF-06's enum, already `[x]` at symbol-map:101-102); the Rust local enum is a faithful mirror used by NodeExecutionResult. Correction recorded.
...
- [x] unit:WF-09 `dag-executor.ts::executeLoopNode` → `har_dag_executor::dag_executor::execute_loop_node()` — **PARITY-VERIFIED sub-cycle 4e (cycle 41, vs live bun 1.3.14 — findings/parity-WF-09-s4e.md)**. Full port of TS:1955-2558 + `DagNode::Loop` dispatch arm. All 11 contract branches differentially confirmed: completion-via-signal, completion-via-`until_bash`-exit0 (real bash), max-iterations→Failed (exact string), interactive gate (InteractiveLoop pause + approval_requested/pending), interactive resume from `metadata.approval` (start_iteration/session threading), fresh_context per-iter, empty-output guard, cost accumulation, paused-tolerance (H4), per-iteration catch (double-wrapped error string oracle-confirmed), 500-UTF-16-unit tool_input truncation (oracle-confirmed byte-exact at non-pair-splitting cut). All pure-string oracles (gate msg ⏸/backticks, plural, SDK double-wrap, empty/max/stop strings, startIteration `(iter??0)+1`, `cleanOutput||fullOutput`) re-derived from running TS and matched. `run_subprocess` env inheritance (`...process.env` + 8 loop keys + `config.envVars` LAST) confirmed faithful. Helpers `build_loop_tool_input`/`truncate_loop_tool_input_value`/`workflow_run_status_str` verified. 4 `[≈]` adjudicated faithful (WF-15 emitter data gap — store events carry full fidelity; provider fail-fast "Original:" text — unreachable display-only; store-error→Failed edge — raw DB text not parity-checkable; UTF-16 lossy lone-surrogate — inexpressible in Rust String, display-only). 11 porter self-tests + 3 verifier durable differential tests (`tests/cycle9_4e_loop_verify.rs`: 500-UTF-16 truncation, until_bash overlay-wins-LAST, exit-nonzero-continues+LOOP_PREV_OUTPUT threading) all green; clippy --tests clean. NOTE: `executeDagWorkflow` symbol-map row stays `- [~]` (rollup) until Approval (4f) lands.
- [x] unit:WF-09 `dag-executor.ts::executeApprovalNode` → `har_dag_executor::dag_executor::execute_approval_node()` — **sub-cycle 4f PARITY-VERIFIED (cycle 42, vs live bun 1.3.14 — findings/parity-WF-09-s4f.md; PASS first pass, no divergence; all 7 probe-battery branches differentially confirmed + 4 pure-string oracles byte-matched vs bun)** — findings/WF-09-s4f-port.md. Full port of TS:2565-2747 + `DagNode::Approval` dispatch arm + `approval_pre_exec_failure` helper (replicates the dispatch catch TS:3387-3416 for substitute/resolve/pause throws). Branches: rejection-resume detection (is_approval_context + type==='approval' + nodeId match), on_reject path (max_attempts ?? 3 exhaustion → cancel + workflow_cancelled event/emitter + cancel msg + Completed; else substitute_workflow_variables + synthetic `${id}:on_reject` PromptNode + resolve + execute_node_internal fresh-session + Failed-passthrough + fall-through), standard gate (substitute_node_output_refs(message) + ⏸ message + approval_requested typed event + pause_workflow_run approval-context + approval_pending emitter + Completed{empty}). 5 Fake-seam tests (`tests/cycle9_4f_approval.rs`) drive the REAL execute_dag_workflow. **Placeholder fully removed: match exhaustive over all 7 DagNode variants, no Skipped `_` arm.** clippy --workspace --all-targets clean; crate suite 444 passed.
...
#### WF-09 — 5B re-harvest reconciliation (pre-DONE sweep, findings/WF-09-s5b-sweep.md)

**Method:** `git kb code symbols --json --limit -1 --file packages/workflows/src/dag-executor.ts` (git-kb 0.2.10) over a FRESHLY RE-INDEXED source (prior index was STALE — 2026-06-05 snapshot, line ranges stopped at 3495 vs current 3710). Re-index → 28 fn/interface/type symbols (was 24 stale). git-kb's TS indexer omits `const`/`let`, so the 8 constants + 3 module caches were enumerated by column-0 grep. Reconciled surface = 28 fn/iface/type + 8 const + 3 cache.

**Rows ADDED for source symbols that had NO symbol-map entry (the silent left-behind class):**

Faithfully ported + exercised by an existing verified parent gate → `- [x]`:
- [x] unit:WF-09 `dag-executor.ts::shellQuote` → `har_dag_executor::dag_executor::shell_quote()` (dag_executor.rs:195) — covered by sub-cycle-1 substituteNodeOutputRefs + 4a bash differential gates
- [x] unit:WF-09 `dag-executor.ts::shellQuoteOrFile` → `har_dag_executor::dag_executor::shell_quote_or_file()` (dag_executor.rs:203) — covered by sub-cycle-1 (file-spill threshold path)
- [x] unit:WF-09 `dag-executor.ts::getEffectiveNodeRetryConfig` → `har_dag_executor::dag_executor::get_effective_node_retry_config()` (dag_executor.rs:507) — covered by 4d retry-loop differential gate
- [x] unit:WF-09 `dag-executor.ts::isTransientNodeError` → `har_dag_executor::dag_executor::is_transient_node_error()` (dag_executor.rs:549) — covered by 4d fatal/transient classification gate
- [x] unit:WF-09 `dag-executor.ts::runStreamPass` (nested in executeNodeInternal) → inlined `'reask: loop` per-pass body (dag_executor.rs:5249-5975) — covered by 4c (11 scripted-provider probes); closure inlined per Rust idiom
- [x] unit:WF-09 `dag-executor.ts::skipIfStatusChanged` (nested in executeDagWorkflow) → `har_dag_executor::dag_executor::skip_if_status_changed()` (dag_executor.rs:4393) — covered by 5A whole-DAG status-change guards
- [x] unit:WF-09 `dag-executor.ts::lastNodeCancelCheck` (module cache) → `last_cancel_check()` throttle map (dag_executor.rs:4699) — covered by 4c cancel-check (H2) probe
- [x] unit:WF-09 `dag-executor.ts::lastNodeActivityUpdate` (module cache) → `last_activity_update()` throttle map (dag_executor.rs:4702) — covered by 4c heartbeat throttle

Idiom-only (no observable behavior) → `- [≈]`:
- [≈] unit:WF-09 `dag-executor.ts::getLog` → Rust `log`/`tracing` macros directly (no cached logger accessor) — pure logging idiom
- [≈] unit:WF-09 `dag-executor.ts::cachedLog` (module `let`) → N/A (Rust tracing has no logger cache) — pure idiom
- [≈] unit:WF-09 `dag-executor.ts::logEventStoreError` (nested in executeLoopNode) → inlined `warn!` on the loop store-event persist-error path — covered by 4e; logging idiom

**GENUINE BEHAVIORAL GAPS — ported-but-INCOMPLETE/STUBBED → `- [!]` blocked (these BLOCK WF-09 DONE):**
- [x] unit:WF-09 `dag-executor.ts::resolveNodeProviderAndModel` → `har_dag_executor::dag_executor::resolve_node_provider_and_model()` (dag_executor.rs:618; CALLED in live dispatch at :4094 + :4375 + approval on_reject :7712) — **PARITY-VERIFIED cycle 45 (parity-WF-09-s5b-resolve.md, vs live bun 1.3.14).** G1–G7 all differentially confirmed: warning strings byte-exact vs the TS templates; capability registry (claude/codex) byte-equal to the real `capabilities.ts`; effort sets + `route_preset_effort` byte-equal; `{...baseOptions, nodeConfig, assistantConfig}` embedding confirmed consumed at both dispatch sites (`resolved.base_options.clone()` → provider call, top-level fields unread); agents copied field-by-field. **Verifier-fixed the fatal-path divergence (nuance 2): a FATAL warning-delivery error now propagates as a resolve `Err` (`Platform authentication/permission error: …`) instead of `.unwrap_or(false)` swallowing it — matching TS `safeSendMessage` rethrow (executor-shared.ts:632-634); the dispatch's resolve-`Err` handler reproduces TS's "failed before execution" node-failure.** Tests: `tests/cycle9_5b_resolve_gaps.rs` (10 probes incl. the new `g3_fatal_delivery_propagates_as_resolve_err`). **PORTED (cycle 45).** All seven gaps closed; signature now threads `&WorkflowLevelOptions`, `platform: &dyn WorkflowPlatform`, `conversation_id`, `workflow_run_id` (matching TS deps; the obsolete `node_fallback_model` param folded into `WorkflowLevelOptions.fallback_model`). (G1) real cap checking — `caps` bound + each `cap_checks` row carries the actual `ProviderCapabilities` flag (`tool_restrictions/hooks/mcp/skills/agents/effort_control/thinking_control/cost_control/fallback_model/sandbox/env_injection`), warn only when `isSet && !cap` (TS:471-498); effort/thinking use real flags + `node.x ?? wlo.x`. (G2/G3) capability warning delivered via `safe_send_message`, byte-matched TS text (TS:505), delivery-fail → `dag.capability_warning_delivery_failed` error log (TS:508-510). (G4) model/provider conflict moved INSIDE the Preset arm with TS's `node.provider && node.provider !== resolved` condition, delivered via `safe_send_message` (TS:434) + `dag.model_provider_conflict_warning_delivery_failed` (TS:438-441). (G5) agents+skills collision delivered via `safe_send_message`, byte-matched (TS:525). (G7) `node_config.hooks = to_value(WorkflowNodeHooks)` (event-keyed map shape). Also completed the full `nodeConfig` build (TS:545-561): effort/thinking/sandbox/betas/output_format/maxBudgetUsd/systemPrompt/fallbackModel/agents (agents via field-by-field `inline_agent_from_def`, NOT a serde round-trip — wire-name mismatch would drop `disallowedTools`/`maxTurns`); and embedded `nodeConfig`+`assistantConfig` INTO `base_options` (TS spread `{ ...baseOptions, nodeConfig, assistantConfig }`) — the dispatch forwards only `base_options`, so without this the resolved config never reached the provider call. Tests: `tests/cycle9_5b_resolve_gaps.rs` (9 fake-seam probes). Delivery-vs-throw nuance: a FATAL platform error is mapped to "not delivered → failure log" (`unwrap_or(false)`) rather than propagated as a resolve `Err` — minor, flagged for verifier. wlo.effort/thinking/betas/sandbox arrive `None` (not yet plumbed through `execute_dag_workflow`'s signature; owed by SV-01 outer caller, same class as code-review #10) — the `node.x ?? wlo.x` precedence is faithfully implemented, SV-01 only needs to populate the struct.
- [x] unit:WF-09 `dag-executor.ts::applyPresetOptions` → `har_dag_executor::dag_executor::apply_preset_options()` (dag_executor.rs ~1089) — **PARITY-VERIFIED cycle 45 (parity-WF-09-s5b-resolve.md).** G6 cascade differentially confirmed: claude `effort:high`→`nodeConfig.effort`, codex `effort:medium`→`assistantConfig.modelReasoningEffort`, codex `effort:max`→`route_preset_effort` None→`dag.preset_effort_unsupported` warn+skip; Rule-1 thinking gating verified. `route_preset_effort` + CLAUDE_EFFORTS/CODEX_REASONING_EFFORTS byte-equal to model-validation.ts. `#[allow(dead_code)]` removed; now mutates `&mut node_config` + `&mut assistant_config` and is WIRED from `resolve_node_provider_and_model` (G6). Cascade matches TS:110-152 exactly: Rule-1 thinking (`preset.thinking` applied to `nodeConfig.thinking` only when node+workflow thinking both unset); Rule-2 early return on unset/already-set effort; Rule-3 `route_preset_effort` with `dag.preset_effort_unsupported` warn on cross-provider mismatch; Rule-4 routes to `nodeConfig.effort` (Effort field) or `assistantConfig.modelReasoningEffort` (ModelReasoningEffort field). Covered by the two `g6_*` cascade probes + the mismatch-dropped probe.
- [≈] unit:WF-09 `dag-executor.ts::WorkflowLevelOptions` → `har_dag_executor::dag_executor::WorkflowLevelOptions` (pub struct: `effort/thinking/fallback_model/betas/sandbox`) — **PARITY-VERIFIED cycle 45 (parity-WF-09-s5b-resolve.md) as `[≈]` (struct + precedence faithful; population deferred).** The struct shape and the `node.x ?? wlo.x` precedence (cap checks effort/thinking/fallbackModel/sandbox; nodeConfig baseline effort/thinking/sandbox/betas/fallbackModel; preset cascade) are differentially verified. **Adjudication (nuance 1):** in TS the values come from the `workflow` param's `& WorkflowLevelOptions` spread (TS:2763,2781-2787 — workflow-definition level); the Rust `execute_dag_workflow` signature plumbs only `fallback_model`, so effort/thinking/betas/sandbox arrive `None`. This is a **faithful honest-deferral of the data SOURCE** (the `execute_dag_workflow` workflow-options signature plumbing / SV-01 outer caller, same class as code-review #10) — NOT a defect of the resolve unit, whose precedence logic is complete and verified. Cross-ref: track WLO population on the `execute_dag_workflow` signature extension (SV-01).

**EXISTING-ROW CORRECTIONS (factual; verifier owns the `- [~]`→final flip):**
- `scheduleReask` (`- [~]` row above): **PHANTOM TARGET** — `schedule_reask()` does NOT exist. The 4-line TS closure is faithfully INLINED at both `'reask: loop` sites (dag_executor.rs:5891-5904 = TS:1227; rs:5930-5943 = TS:1241), exercised by 4c probe 8. Verifier-resolvable to `- [≈]` (closure inlined).
- `NodeExecutionResult` (`- [~]` row above): real target `NodeExecutionResult` struct EXISTS (dag_executor.rs:4729); return type of `execute_node_internal` (4c-verified), cost-carrier central to 4d D-2 fix. Verifier-resolvable to `- [x]`.
- `NodeState` (`- [~]` row above): **WRONG SOURCE CITATION** — `NodeState` appears 0× in dag-executor.ts; it is WF-06's enum (`schemas/workflow-run.ts`, already `- [x]`), locally mirrored as Rust `pub enum NodeState` (dag_executor.rs:4748). Verifier-resolvable to `- [x]`/`- [≈]` after re-citing provenance to WF-06.
