...
- [~] unit:WF-09 `dag-executor.ts::executeDagWorkflow` → `workflows::dag_executor::execute_dag_workflow()` — orchestrator structure parity-verified sub-cycle 2 (cycle 33); dispatch arms: Bash+Cancel verified sub-cycle 4a (cycle 36), Script verified sub-cycle 4b (cycle 37), AI(Command/Prompt) **parity-verified sub-cycle 4d (cycle 40, vs live bun — findings/parity-WF-09-s4d.md; gate FAILed first on D-1 delete-error swallow + D-2 dropped cost + D-3 missing nodeName → all 3 porter-fixed + independently re-verified, 8/8 probes + 4-test durable gate)**; Loop/Approval still honest-Skipped (4e-4f pending → row stays `- [~]` until those arms land)
