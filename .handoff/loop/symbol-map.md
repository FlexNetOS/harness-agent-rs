...
- [!] unit:WF-09 `dag-executor.ts::resolveNodeProviderAndModel` → `har_dag_executor::dag_executor::resolve_node_provider_and_model()` (dag_executor.rs:567; CALLED in live dispatch at :3851 + :4129) — **PARTIAL PORT.** Faithful: provider-registration error (TS:450-457 ✓), model resolution + effective-preset selection ✓. **STUBBED/DROPPED:** (G1) capability-warning logic ignores actual provider caps — `_caps` unbound-unused (rs:650), `cap_checks` carries no capability flag and warns on EVERY set field (rs:696 "simplified — real impl checks caps fields"); TS warns only when `isSet && !caps[cap]` (TS:471-498); effort/thinking checks use wrong stand-in `config_env_vars.is_some()` (rs:677-682). (G2) capability-warning USER delivery via `safeSendMessage` + delivery-fail error log DROPPED (TS:500-511) → Rust log-only. (G4) model/provider-conflict USER delivery via `safeSendMessage` + delivery-fail log DROPPED (TS:421-440) → Rust `warn!` only (rs:610 "Warning delivery would require platform — skip in this utility-only scope"). (G5) agents+skills-collision USER message DROPPED (TS:516-528) → Rust `warn!` only (rs:712). (G7) hooks serialization into `node_config` still deferred (rs:754). Owed: thread platform/conversationId (or move warnings into the dispatch where platform is live) + check `caps[cap]` + deliver via `safe_send_message` + serialize hooks; differential-verify vs a provider lacking a used capability + a node with hooks.
- [!] unit:WF-09 `dag-executor.ts::applyPresetOptions` → `har_dag_executor::dag_executor::apply_preset_options()` (dag_executor.rs:872) — **DEAD CODE** (`#[allow(dead_code)]`, only "caller" is the module doc comment; `resolve_node_provider_and_model` inlines a stub at rs:766-773 `let _ = thinking`). (G6) Rule-1 thinking cascade is a no-op even inside the dead fn; the routed effort it computes is never wired into the executing path → the preset thinking/effort cascade (TS:110-152) is NOT applied for any AI/Loop/Approval node. Owed: wire the preset cascade into `resolve_node_provider_and_model`'s node_config build + differential-verify against a preset carrying thinking/effort.
- [!] unit:WF-09 `dag-executor.ts::WorkflowLevelOptions` (interface: effort/thinking/fallbackModel/betas/sandbox cascade) → threaded as individual option fields into `resolve_node_provider_and_model`; cascade conditions mis-ported (rs:677-682 stand-in). Blocked behind the resolve_node_provider_and_model `- [!]`.
