...
- [~] unit:PR-11 `community/opencode/session.ts::{resolveSessionId(logic),process_message_updated,process_message_part_updated,build_result_chunk,read_structured_output_from_info}` → `har_provider::opencode::session::*` — **`process_message_part_updated` tool branch REGRESSION CLOSED (cycle-20 FINAL re-verify 2026-06-21, 3rd gate pass) — parsing symbols restored to verified; row stays `- [~]` ONLY on the accepted UP-2(b) SDK seam (`client.session.*` + event.subscribe), same posture as copilot/pi.** The `isRecord` guard is restored: `session.rs:313-318` = `.get("input").and_then(match v { Object|Array => Some(clone), _ => None })`, with `skip_serializing_if="Option::is_none"`. Re-diffed vs a FRESH live-bun oracle (lifts `session.ts:200`+`isRecord` verbatim) over the full matrix: object/array→passthrough; **null/string/number/bool/absent→toolInput OMITTED — 0 diffs** (R-OPENCODE-1 null OMIT ✓, R-OPENCODE-2 scalar OMIT ✓ — both closed, no leak). resolveSessionId/message-demux/result-chunk unchanged-PASS (cycle19). multi-agent.ts:258 uses the SAME guard but its live event loop is behind the SDK seam (multi_agent.rs:304 is a test fixture, not a producer) → no second divergent producer. Gate: parity_cycle20_contract_blast.rs (opencode cases all PASS, 0 ignore). NOTE: opencode OMITS (unlike claude/copilot/pi which emit `{}`) — providers NOT homogenized.
