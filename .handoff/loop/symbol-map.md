...
- [x] unit:PR-10 `community/copilot/event-bridge.ts::CopilotEventBridge` → `har_provider::copilot::event_bridge::{map_copilot_event,normalize_copilot_usage,AsyncQueue}` — mapper parity-verified cycle 18 (24-event + 5-usage matrix diffed vs bun: all 8 event types incl. detailedContent>content preference, failure ⚠️/❌ prepend (emoji byte-exact `\u{26A0}\u{FE0F}`/`\u{274C}`/`\u{2699}\u{FE0F}…`), usage capture, session.error deferral+fallback string, reasoning→Thinking, unknown→ignored; 0 diffs). **`[≈]` CLOSED (re-verify 2026-06-21):** `tool.execution_start` with absent `arguments` now maps to `Some(empty map)` → wire `"toolInput":{}` — byte-identical to source `toolInput: args ?? {}` (verified by direct chunk-serialization diff). NOTE: bridgeSession lifecycle (event-bridge.ts:271-434) is the SDK seam (NEEDS-HUMAN), not ported.
...
- [x] unit:PR-11 `community/opencode/session.ts::{resolveSessionId(logic),process_message_updated,process_message_part_updated,build_result_chunk,read_structured_output_from_info}` → `har_provider::opencode::session::*` — cycle19 PASS for these (resume-fallback logic, message demux text/reasoning/tool/tool_result dedup, result chunk tokens+cost+stopReason+modelUsage). NOTE: live `client.session.*` + event.subscribe behind the SDK seam.
